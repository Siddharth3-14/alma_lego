...
buttons_inp = "0100100"
# antennas_inp = "111111111111111111111111111111111111111111111"
antennas_inp = "000000000000000000000000000000000000000000000"
...
def waitforserialchange(ser, bitdict, ant_dict, npadarray=45, verbose=False):
    ...
    while not validhaselection:
        if (NoSerial):
            # buttons = "0110000"
            buttons = buttons_inp
            antennas = antennas_inp

            serialinput = antennas  + buttons
...
#    Flag = False
    ...
    try:
        ...
        # moved above the serial input. Does not cost time anymore.       
            thistime = time.time(); print(("TIMING %f  %f" %(thistime-starttime,thistime-lasttime))); lasttime= thistime
        print('CHECK 1')    
        bit_pos1, bit_pos2, buttons_config,buttons_image,ant_pos, xx_antpos, yy_antpos, singledish,is_there_antenna= waitforserialchange(ser, bitdict, ant_dict, verbose=verbose) 
        if is_there_antenna:
            print('Yes there is antenna')   
        ...
        elif ~is_there_antenna:
            plt.clf()
            pl = plt.subplot(111)
            print('does it get here?')
            pl.text(0.35,0.5,'Please add an antenna',c = 'w')
            print(plt.isinteractive())
            plt.axis('off')
            plt.draw()  #change here 10-2-2026
            plt.pause(2) #change here 10-2-2026
