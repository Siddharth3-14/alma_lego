...
def getserialinterface(deviceroot="/dev/ttyACM", maxdevice=3, boudrate=115200):
    ...
    while True:
        ...
        while count < maxdevice:
            ...
            try:
               ser = serial.Serial(device, boudrate=boudrate)
...
def waitforserialchange(ser, bitdict, ant_dict, npadarray=50, verbose=False):
    ...
    while not validhaselection:
        ...
        # for testing 
        serialinput="001111110001111100011010001110000000000000000000000100010000100" 
        ...
        bit_pos1  = np.where(np.array([bit == '1' for bit in linein]) == True)[0]
        bit_pos2 = np.where(np.array([bit == '1' for bit in linein2]) == True)[0]
        ...
        ant_pos = np.array([np.array(ant_dict[bb])*scale_array for bb in bit_pos1]) #multiply with a factor, default 13 to scale up the array baselines
    ...
    return bit_pos1, bit_pos2, ant_pos, xx_antpos, yy_antpos, singledish
    

def select_model_and_hourangle(bitdict, bit_pos2, verbose=False):
    ...
    if not bitdict['sgal'] in bit_pos2:
        imagefile = "models/toma4_crop.png"
    elif not bitdict['m51'] in bit_pos2:
        imagefile = "models/M51.png"
    elif not bitdict['cam'] in bit_pos2:
...
def get_antenna_dict(filename="ant_pos.txt"):

    #table containing bit,posx,posy of the antennas (also contains ant number, which is not used here)
    ant_bits = ascii.read(filename)

    #a dictionary is created which returns for a given input bit the x,y positions of an antenna
    ant_dict = dict(list(zip(ant_bits['bit'],list(zip(ant_bits['posx'],ant_bits['posy'])))))
...
while True:
    ...
    try:
        ...
        obsMan.select_array('LEGOALMA_Cycle6-C43-9',haStart = hourangle_start,haEnd = hourangle_end,sampRate_s=300)
